...
- **Which inputs to use** (remote media URLs)

- **What outputs files to generate**
...
- Intended for metadata cleanup, not job cancellation

---

## Command Schema Summary

A command object includes:

- `id`: Unique command UUID

- `original_request`: Full request payload used to create the job

- `output_files`: Generated output file mappings

- `status`: Job state

- `total_processing_seconds`: Execution time

- `created_at`: Creation timestamp

- `processing_errors`: FFmpeg stderr output if execution fails
